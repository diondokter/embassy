...
impl WordSize {
    ...
    pub const fn to_hw_size(self) -> Ssize {
        match self {
            WordSize::OneByte => Ssize::EIGHT_BIT,
            WordSize::TwoBytes => Ssize::SIXTEEN_BIT,
            WordSize::FourBytes => Ssize::THIRTYTWO_BYTE,
        }
    }
...
impl AnyChannel {
    ...
    #[inline]
    fn tcd(&self) -> pac::edma_0_tcd0::Tcd {
...
impl<C: Channel> DmaChannel<C> {
    ...
    #[inline]
    pub fn tcd(&self) -> pac::edma_0_tcd0::Tcd {
        // Safety: MCXA276 has a single eDMA instance
        pac::EDMA_0_TCD0.tcd(C::INDEX)
    }

    fn clear_tcd(t: &pac::edma_0_tcd0::Tcd) {
    ...
    #[inline]
    fn set_major_loop_ct_elinkno(t: &pac::edma_0_tcd0::Tcd, count: u16) {
        t.tcd_biter_elinkno().write(|w| w.set_biter(count));
        t.tcd_citer_elinkno().write(|w| w.set_citer(count));
    }

    #[inline]
    fn set_minor_loop_ct_no_offsets(t: &pac::edma_0_tcd0::Tcd, count: u32) {
        t.tcd_nbytes_mloffno().write(|w| w.set_nbytes(count));
    }

    #[inline]
    fn set_no_final_adjustments(t: &pac::edma_0_tcd0::Tcd) {
    ...
    #[inline]
    fn set_source_ptr<T>(t: &pac::edma_0_tcd0::Tcd, p: *const T) {
        t.tcd_saddr().write(|w| w.set_saddr(p as u32));
    }

    #[inline]
    fn set_source_increment(t: &pac::edma_0_tcd0::Tcd, sz: WordSize) {
        t.tcd_soff().write(|w| w.set_soff(sz.bytes() as u16));
    }

    #[inline]
    fn set_source_fixed(t: &pac::edma_0_tcd0::Tcd) {
        t.tcd_soff().write(|w| w.set_soff(0));
    }

    #[inline]
    fn set_dest_ptr<T>(t: &pac::edma_0_tcd0::Tcd, p: *mut T) {
        t.tcd_daddr().write(|w| w.set_daddr(p as u32));
    }

    #[inline]
    fn set_dest_increment(t: &pac::edma_0_tcd0::Tcd, sz: WordSize) {
        t.tcd_doff().write(|w| w.set_doff(sz.bytes() as u16));
    }

    #[inline]
    fn set_dest_fixed(t: &pac::edma_0_tcd0::Tcd) {
        t.tcd_doff().write(|w| w.set_doff(0));
    }

    #[inline]
    fn set_even_transfer_size(t: &pac::edma_0_tcd0::Tcd, sz: WordSize) {
    ...
    #[inline]
    fn reset_channel_state(t: &pac::edma_0_tcd0::Tcd) {
    ...
    pub unsafe fn write_to_peripheral<W: Word>(
        ...
        t.tcd_csr().write(|w| {
            ...
            w.set_inthalf(false);
            w.set_dreq(Dreq::ERQ_FIELD_CLEAR); // Disable request when done
    ...
    pub unsafe fn read_from_peripheral<W: Word>(
        ...
        t.tcd_csr().write(|w| {
            w.set_intmajor(options.complete_transfer_interrupt);
            w.set_inthalf(options.half_transfer_interrupt);
            w.set_dreq(Dreq::ERQ_FIELD_CLEAR); // Disable request when done (important for peripheral DMA)
            w.set_esg(Esg::NORMAL_FORMAT);
            w.set_majorelink(false);
            w.set_eeop(false);
            w.set_esda(false);
